...
public class TramiteService {
    ...
    public static void verificacionR(int idTramite, boolean CMedico, boolean PRealizado, boolean SMultas, String observaciones){
        ...
        TramiteDao.requisitos( idTramite,CMedico,PRealizado,SMultas, observaciones,estado);

    }
    public static Tramite buscarTramitePendientePorCedula(String cedula) {
        Solicitante s = TramiteDao.obtenerSolicitante(cedula);
        if (s == null) return null;

        return TramiteDao.obterTramite(s.getIdSolicitante());
    }

    public static Tramite buscarTramiteEnExamenesPorCedula(String cedula) {
        Solicitante s = TramiteDao.obtenerSolicitante(cedula);
        if (s == null) return null;

        return TramiteDao.obtenerTramiteEnExamenes(s.getIdSolicitante());
    }

    public static double registrarExamenes(int idTramite, double teoria, double practica){
        double promedio = (teoria+practica)/2;
        String estado = promedio>=14 ? "APROBADO" :"REPROBADO";

        TramiteDao.actualizarExamenes(
